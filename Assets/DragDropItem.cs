...
public class DragDropItem : MonoBehaviour
{
    ...
    [Header("Smooth Movement")] public float smoothSpeed = 5f;

    [Header("Distance Constraints")] public float minDistanceToCamera = 2f;
    ...
    void Update()
    {
        ...
        {
            Vector3 targetPosition = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width / 2f, Screen.height / 2f,
    ...
    {
        float distanceToCamera = Vector3.Distance(targetPosition, Camera.main.transform.position);

        if (distanceToCamera < minDistanceToCamera)
        {
            Vector3 direction = (targetPosition - Camera.main.transform.position).normalized;
            targetPosition = Camera.main.transform.position + direction * minDistanceToCamera;
        }

        currentlyDraggedRigidbody.MovePosition(Vector3.Lerp(currentlyDraggedRigidbody.transform.position,
